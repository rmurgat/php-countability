...
function encode_this($string) {
  ...
  $control = "hanygen";                       //defino la llave para encriptar la cadena, cambiarla por la que deseamos usar
...
function decode_get2($string) {
  ...
  $control = "hanygen";                      //defino la llave con la que fue encriptada la cadena,, cambiarla por la que deseamos usar
...
function decode_post2($string) {
  ...
  //procedo a dejar cada variable en el $_POST
  $cad_get = split("[&]",$string);       //separo la url por &
  foreach($cad_get as $value) {
    $val_get = split("[=]",$value);       //asigno los valosres al POST
    $_POST[$val_get[0]]=utf8_decode($val_get[1]);
  }
}
